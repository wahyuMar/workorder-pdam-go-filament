...
#### Item 1 — Clamp Saddle
- **Condition:** generated only if `$survey->clampSaddle` relation is not null
...
#### Item 2 — Crossing
- **Condition:** generated only if `$survey->crossing` relation is not null
...
- Gemini mirror: `GEMINI.md`
- Team rule: keep all files aligned for cross-IDE consistency.
...
5. If the task introduces or clarifies conventions, workflow steps, or guardrails, update `AGENTS.md` first, then sync `.github/copilot-instructions.md` and `GEMINI.md`.
...
- If yes, update `AGENTS.md` and sync `.github/copilot-instructions.md` and `GEMINI.md` in the same task.
