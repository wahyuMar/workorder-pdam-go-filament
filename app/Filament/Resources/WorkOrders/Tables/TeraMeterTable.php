        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('create_spp')
                    ...
                    ->color('success')
                    ->visible(fn ($record) => ! MeterTera::where('complaint_id', $record->id)->exists())
                    ->form(function ($record) {
                        ...
                        return [
                            TextInput::make('no_spp')
                                ->label('No. SPP')
                                ->disabled()
                                ->default(MeterTera::peekNextNoSPP())
                                ->dehydrated(false),
                            ...
                            Textarea::make('keluhan')
                                ...
                                ->disabled()
                                ->dehydrated(),
                            Textarea::make('hasil_tera_meter')
                                ->label('Hasil Tera Meter')
                                ->rows(3)
                                ->required(),
                    ...
                    ->action(function ($record, array $data) {
                        MeterTera::create([
                            ...
                            'hasil_tera_meter' => $data['hasil_tera_meter'] ?? null,
                            'tanggal' => $data['tanggal'],
                        ]);

                        Notification::make()
                            ->title('SPP Tera Meter Berhasil Dibuat')
                            ->success()
                            ->send();
                    })
                    ->modalHeading('Buat Surat Perintah Tera Meter')
                    ->modalWidth('2xl'),
                Action::make('view_spp')
                    ...
                    ->visible(fn ($record) => MeterTera::where('complaint_id', $record->id)->exists())
                    ->form(function ($record) {
                        $spp = MeterTera::where('complaint_id', $record->id)->latest()->first();
